...
class PaymentProofForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "transaction_id": forms.TextInput(attrs={
                "class": "form-control glass-input",
                "placeholder": "Optional — UTR / Transaction ID (if available)",
            }),
            "reference_number": forms.TextInput(attrs={"class": "form-control glass-input", "placeholder": "Optional"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 🌟 Make transaction_id, date, time, amount optional
        ...
        self.fields["method_used"].required = False

    def clean_screenshot(self):
        f = self.cleaned_data.get("screenshot")
        if not f:
    ...
    def clean_transaction_id(self):
        txn_id = (self.cleaned_data.get("transaction_id") or "").strip()
        if not txn_id:
            return ""  # Allow blank transaction ID
