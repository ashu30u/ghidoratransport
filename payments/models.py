...
integration layer / webhook view will be needed.
"""
import uuid
...
class PaymentStatus(models.TextChoices):
    ...
    def color_map(cls):
        return {
            cls.PENDING: "#f0ad4e",
            cls.SUBMITTED: "#5bc0de",
            cls.WAITING_VERIFICATION: "#8e6fee",
            cls.VERIFIED: "#2ecc71",
            cls.REJECTED: "#e74c3c",
            cls.FAILED: "#c0392b",
            cls.EXPIRED: "#7f8c8d",
            cls.REFUNDED: "#3498db",
            cls.COMPLETED: "#16a085",
        }
...
class PaymentRequest(models.Model):
    ...
    preferred_method = models.CharField(
        ...
        help_text="Method the customer ultimately chose."
    )
...
# ---------------------------------------------------------------------------
# PaymentProof (transaction_id, date, time, amount made OPTIONAL)
# ---------------------------------------------------------------------------

class PaymentProof(models.Model):
    payment_request = models.ForeignKey(PaymentRequest, on_delete=models.CASCADE, related_name="proofs")
    method_used = models.CharField(max_length=10, choices=PaymentMethod.choices, default=PaymentMethod.UPI)
    ...
    # 🌟 Transaction ID is now OPTIONAL (blank=True, null=True)
    ...
    def __str__(self):
        txn = self.transaction_id or "No Txn ID"
        return f"Proof for {self.payment_request.payment_id} — {txn}"


# ---------------------------------------------------------------------------
# Verification & Receipts & Logs
# ---------------------------------------------------------------------------
